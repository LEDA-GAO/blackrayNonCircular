...
void raytrace(long double xobs, long double yobs, long double iobs, long double rin, long double disk_length_combined, long double traces[5], int& stop_integration)
{
  ...
  const0 = kt0;
  const1 = r02*s02*kphi0/kt0;//this constant 1 is b=dphi/dt
  ...
  if (stop_integration == 1) { 
    metric(xem[1], Pi/2, met);
    redshift(xem[1], const1, gfactor);
    //cout<<"rtheta1 "<<(kth+kthau)/2.0<<endl;
    ///cout<<" carter "<<-carter/(xem[1]*xem[1])<<endl;
    /*Non Kerr PRD 90, 064002 (2014) Eq. 34*/
    //cosem = -gfactor*kth*sqrt(met[2][2])/(kt0/fact3);
    //The error is because carter and rtheta1 is not exactly same 
    cosem = carter*gfactor/sqrt(xem[1]*xem[1]+epsi3/xem[1]);
    //cosem = carter*gfactor/(sqrt(xem[1]*xem[1]+epsi3/xem[1])*(kt0/fact3));
  }
  else {
    xem[1]  = 0.0;
    gfactor = 0.0;
