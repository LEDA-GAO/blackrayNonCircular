...
std::array<std::array<long double, 4>, 4> metric_rderivativesNC(long double r, long double th)
{
    ...
    t36 = t10 + t11;
    t20 = pow(t15,-5);
    dguudr = -4*t10*t28*t29 + 2*t29*t31 + beta*t10*t19*t20*t26*t5*t8*t9;
    dgppdr = -2*r*t28*t35*(-(t11*(t10 + t11 - 2*r*t29)*t35) + pow(t36,2)) + t31*t35*(4*r*t36 - t11*t35*(2*r - 2*t29 - (beta*t10*t19*t26*t5*t8*t9)/pow(t15,4)));
    dgupdr = 4*spin*t10*t28*t29*t35 - 2*spin*t29*t31*t35 - spin*beta*t10*t19*t20*t26*t35*t5*t8*t9;
    dmn[0][0] = dguudr;
    dmn[3][3] = dgppdr;
    dmn[0][3] = dgupdr;
    dmn[3][0] = dmn[0][3];
